...
const ChatLabel = ({ id, name, openMenu, setOpenMenu }) => {
    ...
    const selectChat = () => {
        ...
        setSelectedChat(chatData)  // Now this should work since setSelectedChat is defined
    ...
    return (
        <div onClick={selectChat} className='flex items-center justify-between p-2 text-white/80 hover:bg-white/10 rounded-1g text-sm group cursor-pointer'>
            ...
            <div onClick={e => { e.stopPropagation(); setOpenMenu({ id: id, open: !openMenu.open }) }}
                ...
                <div className={`absolute -right-36 top-6 bg-gray-700 rounded-xl w-max p-2 ${openMenu.id === id && openMenu.open ? "block" : 'hidden'}`}>
